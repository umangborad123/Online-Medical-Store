...
public partial class SMemail : System.Web.UI.Page
{
    ...
    protected void snbt_Click(object sender, ImageClickEventArgs e)
    {
        ...
        {   
           ...
           while (dr1.Read())
           {
              touid = Convert.ToInt32(dr1[0].ToString());
           }
           dr1.Close();
           q1.Remove(0);

           q1 = "insert into Mail (FromUID, ToUID, FromUser, ToUser, Subject, Message, Date, UploadFile) values ('3', '"+touid+"', '"+fromuser+"', '"+txtto.Text+"', '"+txtsub.Text+"', '"+txtms.Text+"', '"+date+"', '"+filename+"' ) ";
           db.insertData(q1);
           q1.Remove(0);
        }
        else
        {
           ...
           while (dr1.Read())
           {
              touid2 = Convert.ToInt32(dr1[0].ToString());
           }
           dr1.Close();
           q1.Remove(0);

           q1 = "insert into Mail (FromUID, ToUID, FromUser, ToUser, Subject, Message, Date, UploadFile) values ('3', '" + touid2 + "', '" + fromuser + "', '" + txtto.Text + "', '" + txtsub.Text + "', '" + txtms.Text + "', '" + date + "', '" + filename + "' ) ";
           db.insertData(q1);
           q1.Remove(0);
        }
